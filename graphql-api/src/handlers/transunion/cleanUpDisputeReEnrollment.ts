 ...
 */
export const main: Handler = async (event: ScheduledEvent<any>): Promise<any> => {
  const { id } = event;
  ...
  try {
    ...
    const error = errorLogger.createError('reenrollment_operation', 'get_secrets_failure', JSON.stringify(err));
  ...
  try {
    ...
    const error = errorLogger.createError('reenrollment_operation', 'get_certificates_failure', JSON.stringify(err));
    errorLogger.logger.create(error);
  ...
  try {
    ...
    const error = errorLogger.createError('reenrollment_operation', 'unknown_server_error', JSON.stringify(err));
